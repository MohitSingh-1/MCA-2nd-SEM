// To redirect the standard input use command: Get-Content rno42_bfs_input_mohit.txt | .\rno42_bfs_mohit.exe
...
// BFS function to cover all component
void bfs(vector<vector<int>>& adj,int V,int source){
    cout<<"BFS output : ";
    vector<int> vis(V,-1);  // visited array which represents -1:White , 0:Black, and 1:Grey

    queue<int> q;
    q.push(source);
    vis[source] = 1;    // marking the source as grey
    ...
    while(!q.empty()){
        ...
        for(auto neighbor:adj[curr_node]){
            if(vis[neighbor]==-1){      // if neighbor is not visited then adding to the queue
                q.push(neighbor);
                vis[neighbor] = 1;      // marking neighbor as grey 
            }
        }
        
        vis[curr_node] = 0;     // marking the current node black as it is processed and popped from the queue.
    }
}
...
int main(){
    ...
    bfs(adj,V,0);       // passing 0 as source node 
